<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="TX_Order.aspx.vb"
Inherits="BVAC_API_C.TX_Order" %>
      ...
      <div id="q-app">
        ...
        <div class="bg_black">
                ...
                >
                  <a class="dropbtn">
                    ...
                    <template v-else-if="this.$i18n.locale == 'en'">
                      <div
                        ...
                        "
                      ></div>
                      <div style="display: inline-block; width: 41px">
                        English
                      </div>
                    </template>
                  ...
                  <div class="dropdown-content">
                    ...
                    <a
                      ...
                      v-on:click.prevent.stop="change_language('zh')"
                      ><div class="flag_cn"></div>
                      中文</a
                    >
                    <a
                      v-if="this.$i18n.locale != 'en'"
                      v-on:click.prevent.stop="change_language('en')"
                      ><div class="flag_en"></div>
                      English</a
                    >
                    <a
                      v-if="this.$i18n.locale != 'jp'"
                      v-on:click.prevent.stop="change_language('jp')"
                      ><div class="flag_jp"></div>
                      日本語</a
                    >
                  </div>
                </div>
            ...
            <div class="content02">
              <div v-if="step < 50" class="blue01" style="text-align: left">
                ...
                <template v-if="step == 40">
                  ...
                  <div style="text-align: left; padding-left: 10px">
                    <h4 style="margin: 0">
                      {{ $t("lan.remindTimeOfOrderPayment") }}
                    </h4>
                    <h1>{{ minutes }}分{{ seconds }}秒</h1>
                  </div>

                  <h4 style="text-align: center">{{ $t("lan.longlong") }}</h4>
                  <div class="blue_frame" style="display: block">
                    {{ $t("lan.payAmt") }}：<span
                      style="color: #1f7eff; font-size: 1.6em; font-weight: 500"
                      >{{ amt }}</span
                    >
                  </div>
                  <div class="blue_frame" style="display: block">
                    {{ $t("lan.receiverName") }}：{{ rname }}
                    <img
                      v-on:click="copy(rname)"
                      src="images/icon_copy.png"
                      style="float: right"
                    />
                  </div>
                  <div class="blue_frame" style="display: block">
                    {{ $t("lan.receiverAcc") }}：{{ account_no
                    }}<img
                      v-on:click="copy(account_no)"
                      src="images/icon_copy.png"
                      style="float: right"
                    />
                  </div>
                  <div class="blue_frame" style="display: block">
                    {{ $t("lan.bankOfAcc") }}：{{ account_bank
                    }}<img
                      v-on:click="copy(account_bank)"
                      src="images/icon_copy.png"
                      style="float: right"
                    />
                  </div>
                  <div class="blue_frame" style="display: block">
                    {{ $t("lan.LocateState") }}：{{ account_bank_city }}
                    <img
                      v-on:click="copy(account_bank_city)"
                      src="images/icon_copy.png"
                      style="float: right"
                    />
                  </div>
              ...
              <template v-if="step == 15">
                <div id="currency">
                  ...
                  <div
                    v-for="(obj) in rechargeCurrencyList"
                    style="display: inline-block"
                  >
                    <button
                      v-if="obj.isSelectable == true"
                      :key="obj.submit_id"
                      :class="obj.style_class"
                      v-on:click.stop.prevent="selectRechargeCurrency(obj)"
                    >
                      <div
                        class="icon_02"
                        :style="'background-image: url(images/icon_' + obj.code + '.png);'"
                      ></div>
                      <p>{{ $t(obj.currency_name_chn) }}</p>
                    </button>
                    <button
                      v-if="obj.isSelectable == false"
                      :key="obj.paymentmethod_id"
                ...
                <div style="margin-bottom: 1.5em">
                  <div class="l_side" style="">
                    <div>
                      ...
                      <input
                        type="text"
                        :placeholder="$t(extra_info_placeHolder)"
                        v-model="recharge_addr"
                        v-on:input="checkDisableNextStep(step,'top up addr')"
                        autocomplete="off"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        :placeholder="$t(extra_info_placeHolder)"
                        v-model="recharge_amt"
                        v-on:input="checkDisableNextStep(step,'top up amt')"
                        autocomplete="off"
                      />
                    </div>
                  </div>
              ...
              <template v-if="step >= 10 && step < 20">
                <div id="payment_method" v-if="step == 10">
                  ...
                  <div style="display: inline-block">
                    <button
                      :class="(haveWallet==true)?btn_selected_style:btn_normal_style"
                      v-on:click.stop.prevent="selectWithWallet(true)"
                    >
                      <div
                        class="icon_02"
                        :style="'background-image: url(images/icon_havewallet.png);'"
                      ></div>
                      <p>我已有數字貨幣錢包</p>
                    </button>
                  </div>
                  <div style="display: inline-block">
                    <button
                      :class="(haveWallet==false)?btn_selected_style:btn_normal_style"
                      v-on:click.stop.prevent="selectWithWallet(false)"
                    >
                      <div
                        class="icon_02"
                        :style="'background-image: url(images/icon_nowallet.png);'"
                      ></div>
                      <p>無數字貨幣錢包</p>
                    </button>
                  </div>
                ...
                <template v-if="haveWallet==true">
                  <div id="payment_method" v-if="step == 11">
                    <h4>轉/買幣</h4>
                    <div
                      v-for="(obj) in this.paymentMethodJSON"
                      style="display: inline-block"
                    >
                      <button
                        v-if="obj.isSelectable == true"
                        :key="obj.paymentmethod_id"
                        :class="obj.style_class"
                        v-on:click.stop.prevent="selectPaymentMethod(obj)"
                      >
                        <div
                          class="icon_02"
                          :style="'background-image: url(images/icon_' + obj.code + '.png);'"
                  ...
                  <div
                    id="currency"
                    :style="currencyDivStyle"
                    v-if="step == 12"
                  >
                    <h4>{{ $t("lan.currency") }}</h4>
                    <div
                      v-for="(obj) in this.currencyList"
                      style="display: inline-block"
                    >
                      <button
                        v-if="obj.isSelectable == true"
                        :key="obj.submit_id"
                        :class="obj.style_class"
                        v-on:click.stop.prevent="selectCurrency(obj)"
                      >
                        <div
                          class="icon_02"
                          :style="'background-image: url(images/icon_' + obj.code + '.png);'"
              ...
              <template v-if="step == 20">
                <div style="margin-top: 1.5em">
                  ...
                  <div
                    ...
                    v-for="(bankObj) in this.bankList"
                    style="display: block"
                  >
                    <button
                      v-if="bankObj.avb"
                    ...
                    >
                      <div :class="'icon_MYR_' + bankObj.code"></div>
                      <p>{{ bankObj.name }}</p>
                    </button>
                    <button
                      v-if="!bankObj.avb"
                  ...
                  <div class="l_side">
                    ...
                    <input
                      ...
                      :placeholder="$t(extra_info_placeHolder)"
                      v-model="pname"
                      v-on:input="checkDisableNextStep(step, 'pname')"
                      autocomplete="off"
                    />
                  </div>
                  ...
                  <div class="r_side">
                    <h4 style="line-height: 1.5rem">
                      {{ $t("lan.followInsturctionAndPayEnoughAmt") }}
                    </h4>
              ...
              <template v-if="step == 50">
                <div class="content02" style="text-align: center">
                  ...
                  <p style="font-weight: 400">
                    {{ $t("lan.announcementAfterTranaction") }}
                  </p>
                  <p style="margin-bottom: -30px">
                    {{ $t("lan.announcementAfterTranactionTime") }}
                  </p>
                </div>
              </template>
            </div>
            <template v-if="step == 40">
              <p style="font-size: 1.3em; color: #fff; text-align: center">
                {{ $t("lan.prevStep") }}
              </p>
              <div>
                <p
                  style="
                    ...
                    border-top: 1px solid rgb(185 185 185 / 30%);
                    padding: 30px 0 0;
                  "
                >
                  <span style="color: #ff1c1c"
                    >{{ $t("lan.no1") }}{{ $t("lan.unionPayRules1") }}</span
                  ><br />
            ...
            <q-dialog v-model="alert" persistent="true">
              <div>
                <div v-if="isWaitingForRetry" class="close01">
                  <a href=""
                    ><img src="images/close_icon.png" style="display: block"
                  /></a>
                </div>
                <div v-if="!isWaitingForRetry" class="popup01">
                  <div>
                    <img
                      src="images/search_icon01.png"
                      style="
                        display: block;
                        text-align: center;
                        margin: auto;
          ...
          <footer>
            ...
            <p>
              Copyright © 2019 BVAC All rights reserved and shall not be
